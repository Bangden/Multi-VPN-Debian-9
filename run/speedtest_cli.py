...
def speedtest():
    ...
    if not args.simple:
        print_('Mengambil konfigurasi speed.bangden.codes...')
    try:
        config = getConfig()
    except URLError:
        print_('Tidak dapat mengambil konfigurasi speed.bangden.codes')
        sys.exit(1)

    if not args.simple:
        print_('Mengambil daftar server speed.bangden.codes...')
    ...
    if not args.simple:
        print_('Menguji dari %(isp)s (%(ip)s)...' % config['client'])
    ...
    if args.server:
        ...
        if not args.simple:
            print_('Memilih server terbaik berdasarkan latensi by Bang Den...')
        best = getBestServer(servers)
